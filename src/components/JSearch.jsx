...
function Search({ collection }) {
  ...
  async function handleOnSearch({ target = {} }) {
    const { value } = target;
    ...
    <div>
      ...
      <ul className="list-none">
        {posts &&
          posts.map((post) => (
            <li className="py-2">
              <a className="text-lg text-blue-700 hover:text-blue-900 hover:underline underline-offset-2" href={`/${post.collection}/${post.slug}`}>{post.title}</a>
